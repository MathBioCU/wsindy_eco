...
% DAM - edits to ksdensity to allow box kernel, bounded domain 
%%
function[h,L,MX,MED,bw]=violin_DAM(Y,varargin)
...
if isempty(find(strcmp(varargin,'kernel')))==0
    ker = varargin{find(strcmp(varargin,'kernel'))+1};
end
...
for i=1:size(Y,2)
    ...
    if isempty(b)==0
        [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'kernel',ker,'support',supp,'BoundaryCorrection','reflection');
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{i});
    end
    
    % f=f/max(f)*0.3; %normalize
    % f = f/max(f)*max(diff(x))/4;
    ideal_norm = 1/max(f)*max(diff(x))*0.49;
    f = f*min(ideal_norm,1/norm(f,1)/mean(diff(u))*range(u)*max(diff(x))/14);
    F(:,i)=f;
    U(:,i)=u;
    MED(:,i)=median(Y{i},[],'omitnan');
    MX(:,i)=mean(Y{i},[],'omitnan');
    bw(:,i)=bb;
...
for i=i:size(Y,2)
    if isempty(lc) == 1
        ...
        if setX == 0
            h(i)=fill([F(:,i)+i;flipud(i-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc,'linewidth',lw);
        else
            h(i)=fill([F(:,i)+x(i);flipud(x(i)-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc,'linewidth',lw);
        end
    end
    hold on
    if setX == 0
        if plotmean == 1
            p(1)=plot([x(i)-max(diff(x))/4,x(i)+max(diff(x))/4],[MX(:,i) MX(:,i)],mc,'linewidth',lw);
            % p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'linewidth',lw);
        end
        if plotmedian == 1
            p(2)=plot([x(i)-max(diff(x))/4,x(i)+max(diff(x))/4],[MED(:,i) MED(:,i)],'--','color',medc,'linewidth',lw);
            % p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i)) ],[MED(:,i) MED(:,i)],'--','color',medc,'linewidth',lw);
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([x(i)-max(diff(x))/4,x(i)+max(diff(x))/4],[MX(:,i) MX(:,i)],mc,'linewidth',lw);
        ...
        if plotmedian == 1
            p(2)=plot([x(i)-max(diff(x))/4,x(i)+max(diff(x))/4],[MED(:,i) MED(:,i)],medc,'linewidth',lw);
