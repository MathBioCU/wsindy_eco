...
for ind = 1:2
    ...
    for j=1:n
        if length(X_pred_cloud{j}(:,ind))>=3
            dat = X_pred_cloud{j}(1:min(max_gen_cap,end),ind);
            [AM{j,ind},PD{j,ind}] = findpeaks(dat,'MinPeakDistance',min(floor(length(dat)/2),4));
            len(j,ind) = length(AM{j,ind});
        end
    end
end

mus_P = zeros(max(len(:)),nstates); mus_A = zeros(max(len(:)),nstates);
for ind = 1:nstates
...
view_ind = test_ind;
X_pred = X_pred_cell{view_ind}; 
X_test = X_test_cell{view_ind}; 
Y_pred = cell2mat(Y_pred_cell{view_ind});
Y_test = cell2mat(Y_test_cell{view_ind});
t_pred = t_pred_cell{view_ind};
t_test = t_test_cell{view_ind};
tn_test = (0:size(X_test,1)-1)*yearlength;
...
% jj=1;
% tn_test = (0:size(X_test_cell{jj},1)-1)*yearlength;
% t_epi_test = t_epi(1:size(X_test_cell{jj},1)-1);
...
for j=1:nstates
    ...
    h2=plot(tn_pred,X_pred(:,j),xL_cl,...
        t_pred, Y_pred(:,j),yL_cl,...
        yearlength*[n_err_tol]*[1 1],ylims{j},'k--',...
        'linewidth',3,'markersize',7);
    hold on
    h0=plot(tn_test,X_test(:,j),x_cl,...
        t_test, Y_test(:,j),'--',...
        yearlength*[n_err_tol]*[1 1],ylims{j},'k--',...
        'linewidth',3,'markersize',7);
     set(h0(2),'color',y_cl)
    ...
    set(gca,'ticklabelinterpreter','latex','fontsize',20,...
        ...
        'Yscale',yscl,...
        'Ylim',ylims{j},...
        'Ytick',yticks{j},'Xlim',[0 max(t_test)])
    grid on;

    legend([h0(2);h2(2);h3],{'true model output','learned model output',[num2str(clev*100),'% CR']},...
        'location','sw','interpreter','latex','fontsize',16)
    if j==1
...
% for k=ks'
%     i = train_inds(X_in(k));
%     h0=plot(t_epi_test{i},Y_test_cell{view_ind}{i}(:,j),'markersize',7,'linewidth',4);
