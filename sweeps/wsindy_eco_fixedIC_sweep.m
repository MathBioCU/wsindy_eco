...
for kk=1:length(snr_Ys)
    ...
    mods_cell = cell(length(ntrain_inds),length(rngs));
    ...
    for ii=1:length(ntrain_inds)
        parfor jj=1:length(rngs)
            ...
            results_cell{ii,jj} = [0,0,0,...
                errs_2_Y,errs_inf_Y,tpr_Y,...
                errs_2_X,errs_inf_X,tpr_X,RT,n_err_tol];

            sim_cell{ii,jj} = {X_test_temp,X_pred_temp};
            maps_cell{ii,jj} = {rhs_Y,rhs_X};
            coeffs_cell{ii,jj} = {W_Y,W_X};
            mods_cell{ii,jj} = {WS_Yeq,WS_Xeq};
            libs_cell{ii,jj} = cellfun(@(L)cell2mat(L.tags'),{lib_Y_Yeq,lib_X_Yeq,lib_Y_Xeq,lib_X_Xeq},'uni',0);
